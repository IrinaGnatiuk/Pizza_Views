...
for row in mydivs:
    ...
    description = row.find_all('div',
                               {'class': 'catalog-card__composition'})[0]
    image = row.find_all(
        'div',
        {'class': 'catalog-card__img-static js-catalog-card__img-static'})[0]
    values = [
        name.h2.contents[0],
        int(price.p.contents[0]),
        description.p.contents[0],
        image.picture.img.get('data-src')
    ]
